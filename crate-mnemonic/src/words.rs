pub const LEN_WORD_MAX: usize = 8;

pub const ALL_MNEMONIC_WORDS: [&str; 2048] = [
...
mod tests {
    ...
    #[test]
    fn ensure_english_words_is_capped_correctly() {
        let mut max_word_len = 0;

        for word in ALL_MNEMONIC_WORDS {
            max_word_len = core::cmp::max(max_word_len, word.len());
        }

        assert_eq!(max_word_len, LEN_WORD_MAX);
    }

    #[test]
    fn idiomatically_ensure_english_words_is_capped_correctly() {
        let longest_word = ALL_MNEMONIC_WORDS.iter().max_by(|x, y| x.len().cmp(&y.len())).unwrap();

        assert_eq!(longest_word.len(), LEN_WORD_MAX);
    }
}
